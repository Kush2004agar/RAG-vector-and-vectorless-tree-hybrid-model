...
At a high level, the system is a **hybrid hierarchical RAG** pipeline:
...
### `ModuleNotFoundError: No module named 'dotenv'` even after install

This almost always means `pip` installed into a **different Python** than the one running your scripts.

Use the venv’s Python explicitly:

```powershell
.\.venv\Scripts\python -m pip install python-dotenv
.\.venv\Scripts\python run_qa_pipeline.py -i "Network Security and 5G Questions.xlsx"
```

If your venv has no pip, repair it:
